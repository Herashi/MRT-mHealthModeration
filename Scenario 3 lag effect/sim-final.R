...
rsnmm = function(n, T,
  ...
  for (i in 0:(n-1)){
    ...
    for (j in 2:T){
      state_mean = mean(state[(g_member-1)*T+j-1])
...
rsnmm.control <- function(origin = 1, sd = 1,
                          ...
                          beta0 = c(-0.2, 0, 0, 0.2, 0), beta1 = c(-0.1,0,0,0.2),
                          eta = c(0, 0, 0.8, -0.8, 0), mu = rep(0, 3),
                          theta0 = c(0, 0.8), theta1 = c(0, 0),
                          coef.avail = c(100, rep(0, 3)), coef.state = rep(0, 5),
                          tfun = NULL, lag = 3 + any(beta1 != 0)) { #if any entry in beta1=0, there will be lag effect
...
sim_wc <- function(n = 100, tmax = 30, M = 1000,
                   ...
                   y.formula = list(w = y ~ state + I(lag1a-pn)),
  ...
  fitter <- function(formula, args, prob, coef, label, response = "y",
    ...
    if (!is.null(addvar)) {
      ...
      est <- estimate(fit,group= group_ls, rbind("Average lag 1 effect" = contrast_vec))[,1:4]
      ...
      fit$vcov <- vcov.geeglm(fit, group  = group_ls)
      estc <- estimate(fit, group= group_ls,rbind("Average lag 1 effect" = contrast_vec))[,1:4]
      fit <- data.frame(moderator = c("Average lag 1 effect"), 
  ...
  out <- foreach(m = 1:M, .combine = "rbind") %dopar% {
    ...
    fity <- data.frame(iter = m, true = -0.1,
