...
rsnmm = function(n, T,
                 ...
                 y, err, statec, ac, availc, group_err, slope){
...
sim_wc <- function(n = 30, tmax = 30, M = 1000,
                ## response regression models
                y.formula = list(w = y ~ state + I(a - pn) * (Group_1 + Group_2+ Group_3+ Group_4+ Group_5)),
                contrast_vec = c(0,0,6,rep(0,5),rep(1,5))/6,
  ...
  fitter <- function(formula, args, prob, coef, label, response = "y",
    ...
    if (!is.null(addvar)) {
      ...
      fit <- data.frame(moderator = c("Average group treatment effect"), #truth = truth,
  ...
  for (m in 1:M){
    ...
    fity <- data.frame(iter = m, method = "Weighted and centered",
                         fity, row.names = NULL)
    out = rbind(out,fity)
  }
  ...
  out <- cbind(aggregate(cbind(est, se, sec, cp, cpc, rmse,lclc, uclc) ~
                           method + moderator +  n + tmax,
                         data = out, FUN = mean),
               sd = aggregate(est ~ method + moderator + n + tmax,
                              data = out, FUN = sd)$est,
