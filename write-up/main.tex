...
In many areas of behavioral science, push notifications in mobile apps adapted to continuously collected information on the individual's current context have received a considerable amount of attention.  These time-varying adaptive interventions are hypothesized to lead to meaningful short- and long-term change towards healthier behaviors. The assessment of the time-varying effect of such push notifications motivated sequential randomized designs such as micro-randomized trials (MRT) \cite{Nahum2017, KlasnjaMRT}. In an MRT, interventions are randomized to be potentially delivered at a over a period of time, at hundreds and thousands of decision points. The MRT design enables the estimation of the time-delayed marginal treatment effects of a push notification upon the outcome of interest at multiple time-lags, referred to as ``causal excursion effects" \citep{Qian2021}. A weighted, centered least squares (WCLS) approach has been developed to consistently estimate causal excursion effects with robust methods of uncertainty assessment. Extensions to balance the number of randomizations by observed or derived individual-level contextual information have also been developed, e.g., a stratified MRT~\cite{Dempsey_AOAS}.
...
%\zw{Can use terminologies such as peer effects, unobserved confounders and homophily if needed.}
% WD: for now , I am trying to avoid network terms to not lead reviewer to think "network" and more to let them think "clustering"
%for groups of subjects from a social network.\zw{should we just say clusters of individuals or explicitly mention social network. But it will be fine if this methods can factor in an observed netwrok structure.}  WD: I think we avoid network discussion for now and leave that for the 

The primary contribution of this paper is the significant extension of data analytic methods for MRTs to account for potential cluster-level correlation and the extension of the causal excursion effect to account for potential interference, i.e., we define both a direct and indirect causal excursion effect.  In Section~\ref{section:preliminaries}, we review the MRT design and the existing method's implicit assumptions.  In Section~\ref{lemma:cond_effect}, we introduce the moderated causal excursion effect using a group-based conceptualization of potential outcomes as seen in~\cite{Hong2006, Vanderweele2013}.  We then extend the weighted, centered least squares criterion to estimate both direct and indirect causal excursion effects in Section~\ref{section:standardmrtmethods}.  Finally, we demonstrate that when proximal outcomes exhibit cluster-level correlation but there is an absence of treatment effect heterogeneity, our proposal agrees with the traditional approach both in terms of point estimate and asymptotic variance.  In Section~\ref{section:casestudy}, we illustrate the proposed method by estimating the causal excursion effects of push notifications on mood, sleep, and step count for a cohort of first year medical residents in the United States. The paper concludes with a brief discussion.

% \zw{Add a paragraph to carve out the major contributions; distinguish from more advanced large social network setting.}
...
$I_t = 1$ if available for treatment and $I_t = 0$ otherwise. Availability at time $t$ is determined before treatment randomization. When an individual is unavailable for treatment ($I_t = 0$), then no treatment is provided ($A_t = 0$). In this paper, for simplicity, individuals are assumed always available for treatment. All methods can be readily extended to account for availability as in \citep{Boruvkaetal}. 
...
Following \citep{Boruvkaetal, Qian2021,Dempsey_AOAS}, we focus on the class of estimands that is referred to as ``causal excursion effects" defined by
$$
\beta (t;s) = \E \left[ \E \left[ Y_{t+1} \given H_t, A_t = 1 \right] - \E \left[ Y_{t+1} \given H_t, A_t = 0 \right] \given S_t = s\right]
$$
where $S_t$ is a time-varying potential effect moderator (or ``state") and a deterministic function of the observed history $H_t$. For example, in HeartSteps~\cite{HeartSteps2018}, $S_t$ may represent contextual information experienced by a subject such as step counts in the previous 30 minutes. 
...
\arg \max_{\alpha, \beta} \mathbb{P}_n \left[ \sum_{t=1}^T w_t (H_t) \left( Y_{t+1} - g_t(H_t)^\prime \alpha - (A_t - \tilde p_t (1\textbf{} \mid S_t)) f_t (S_t)^\top \beta \right) \right]
\end{equation}
where~$\mathbb{P}_n$ is defined as the average of a function over the sample, $w_t(H_t) = \tilde p_t (A_t \mid S_t) / p_t (A_t \mid H_t)$ is a weight where the numerator is an arbitrary function with range $(0,1)$ that only depends on $S_t$ and the denominator is the MRT specified randomization probability, and $g_t(H_t) \in \mathbb{R}^p$ are a set of control variables chosen to help reduce the variance of the estimators.  The control variables are chosen to construct more powerful test statistics. In HeartSteps, for example, a natural control variable is the number of steps in the prior thirty minutes as this variable is likely highly correlated with the proximal response and thus can be used to reduce variance in the estimation of $\beta$. See~\cite{Boruvkaetal} for additional details on how to define and estimate moderated time-varying treatment effects using data from a  micro-randomized trial.  
  This effect is often referred to as the `causal excursion effect'~\cite{Dempsey_AOAS}. 
...
Given the decomposition, in such settings, three immediate questions may arise: (1) does the standard MRT analysis have good statistical properties when participants exhibit cluster-level correlation?
% \zw{demonstrate such correlations are non-trivial. Need to discuss.}
(2) can one assess potential cluster-level effect moderators using the standard MRT analysis? and (3) can one extend the MRT analysis to assess the effect of treatment provided to one individual on the proximal response of another individual in the same cluster?  For (1), we demonstrate in Section~\ref{section:sims} that cluster-level treatment effect heterogeneity leads to decay in coverage of the true moderated effect by the $95\%$ confidence interval obtained from standard weighted, centered least squares methods. For (2), in IHS, for example, a scientific question may be how group average mood impacts the proximal effect of notifications on average daily mood.  Since each individual's mood may be impacted by their own treatments, if the proximal effect is moderated by cluster-level summary variables then this implies the proximal response is a function of past treatment assignment to all individuals in the group.  For (3), in Section~\ref{section:prox_effects_pot_outcome} we extend the causal excursion effect to distinguish between direct and indirect causal excursion effects.

Answering these three questions requires generalizations of existing methods to account for clustered outcomes and potential within-cluster interference of other subjects' treatments upon one subject's outcome. Here we define a \emph{cluster MRT} to be a micro-randomized trial in which there is a pre-defined cluster-level structure.  The cluster structure may result in (A) cluster-level correlation among proximal responses (question 1), (B) treatment impacting individual-level moderators which then impact proximal responses of other individuals (question 2), or (C) treatment interference, i.e., treatment provided to one individual may impact the proximal response of another individual in the same cluster (question 3).  We will demonstrate how our proposed method addresses these three issues.
...
\subsubsection{Direct causal effects}
In standard MRTs, the individual is the unit of interest.  In cluster MRTs, the cluster is the unit of interest.  At the cluster level, our interest lies in the effect of providing treatment versus not providing treatment at time $t$ on a random individual in the group.  This can be expressed as a difference in potential outcomes for the proximal response and is given by
\begin{equation}
...
\end{equation}
Equation~\eqref{eq:directeffect} is a \emph{direct causal effect} of treatment compared to no treatment on the proximal response for the same individual, while all previous treatments on the individual and all treatments on every other member of the cluster are fixed to a specific value.
% \zw{clarify the directness means blocking all other paths except the path from $A_{tj}$ to $Y_{t+1,j}$}

The ``fundamental problem of causal inference''~\cite{Rubin, Pearl2009} is that these individual differences cannot be observed. Thus, similar to prior work~\cite{Dempsey_AOAS, Boruvkaetal}, in this paper averages of potential outcomes are considered in defining treatment effects. Let $S_t (\bar a_{t-1})$ denote a vector of potential moderator variables chosen from $H_t (\bar a_{t-1})$, the cluster-level history up to decision point $t$.  We write $S_t (\bar a_{t-1}) = \left( S_{t,j} (\bar a_{t-1}), S_{t,-j} (\bar a_{t-1}) \right)$ to clarify that the potential moderator variables can contain both information on the selected individual as well as other individuals in the cluster. Then moderated direct treatment effect, denoted $\beta (t; s)$, can be defined as
\begin{equation}
...
$$
\frac{1}{G \cdot (G-1)} \sum_{j^\prime \neq j} \left[Y_{t+1,j} (\bar a_{t,-\{j, j^\prime\}}, (\bar a_{t-1,j}, 0), (\bar a_{t-1,j^\prime}, 1)) - Y_{t+1,j} (\bar a_{t,-\{j, j^\prime\}}, (\bar a_{t-1,j}, 0), (\bar a_{t-1,j^\prime}, 0)) \right].
$$
...
\end{array}
\end{equation}
where $J^\prime$ is uniformly distributed random index on the set $[G] \backslash \{J\}$. The expectation is again over both the potential outcomes~($Y_{t+1,J}(\cdot)$), the set of randomized treatments~($\bar A_{t,-\{J,J^\prime\}}$, $\bar A_{t-1,J}$, and $\bar A_{t-1,J^\prime}$), and the random indices~($J$ and $J^\prime$). Here, the potential moderator can be written $S_t (\bar A_{t-1}) = \left( S_{t,J} (\bar A_{t-1}), S_{t,J^\prime} (\bar A_{t-1}), S_{t,-\{J, J^\prime \}} (\bar A_{t-1}) \right)$ to clarify that the variables can contain both information on the two selected individuals as well as others in the cluster. Note that a similar effect can be defined when the individual does receive treatment. For now, we focus on the effect defined by~\eqref{eq:inddirectavglineareffect}.

% \subsubsection{Total causal effects (necessary?)}


\subsubsection{Estimand dependence on treatment distribution}

In this section, we clarify the differences among the above estimands, the traditional MRT estimand~\cite{Boruvkaetal}, and estimands commonly studies in longitudinal treatment effect estimation~\cite{Robins}. In the causal inference literature, the typical estimand is an expected outcome for a particular sequence of treatments,i.e.,~$\E \left[ Y(a_1,\ldots, a_T)\right]$.  Such estimands do not depend on the treatment distribution, but are often not of primary interest in the current setting since many sequences of treatment may never be observed in finite samples given the large number of decision points (often hundreds or thousands).

The estimands considered in this paper are most similar to average outcomes under a particular dynamic treatment regime~$\E_\pi \left[ Y(A_1,\ldots, A_T)\right]$ where $\pi$ denotes the dynamic treatment regime from which the treatments are drawn~\cite{MurphyJASA2001}. Indeed, for any $A_{u,j}$ not contained in $S_t (\bar A_{t-1})$, the direct and indirect effects depend on the distribution of $\{ A_{u,j} \}_{u < t, j \in [G]}$.  Estimands~\eqref{eq:directeffect} and~\eqref{eq:inddirectavglineareffect} marginalize over treatments not contained in $S_t(\bar A_{t-1})$ under the micro-randomized distribution.  Marginalization over different probabilistic assignment of treatments may yield different results. Therefore, the marginal direct and indirect effects depend on the study protocol. The reason for this is that micro-randomization is meant to gather information on how to optimize the design of intervention components.  Therefore, the formulation of main and moderation effects aim to help scientists improve particular intervention components.  Regardless, the effects considered in this paper are causal and depend on the cluster-level treatment distribution.  Due to this dependence, we recommend presenting the micro-randomization distribution together with the estimated treatment effects.

\subsection{Proximal effect of treatment \& observable data}
...
\noindent In practice, plug-in estimates $\hat Q$ and $\hat W$ are used to estimate the covariance structure. Appendix~\ref{app:ssa} presents small sample size adjustments that are used in the analysis. 
...
\end{equation}
where $\tilde p_t^\star (1\mid S_t) = \frac{\tilde p_t (0,1 \mid S_t)}{\tilde p_t (0,0 \mid S_t) + \tilde p_t (0,1 \mid S_t)}$.  If an individual's randomization probabilities only depends on their observed history then $\tilde p_t^\star (1 \mid S_{t,j^\prime}) = \tilde p_t (1 \mid S_{t,j^\prime})$.  In Appendix~\ref{app:techdetails}, we prove the following result.

\begin{prop}
\label{lemma:asymnorm2}
Under assumption~\ref{ass:indirecteffect}, then, under invertibility and moment conditions, the estimator $\hat \beta$ that minimizes \eqref{eq:directwcls} satisfies $\sqrt{M} \left( \hat \beta^{(IE)} - \beta^{\star \star} \right) \to N(0, Q^{-1} W Q^{-1})$ where
$$
...
\noindent In Appendix~\ref{app:addindirect}, the above variances are re-derived in the general setting where the numerators $\tilde p_t (a \mid S_t)$ and $\tilde p_t (a, a^\prime \mid S_t)$ are estimated using the observed MRT data.
...
\end{equation}
for some function $\psi$, i.e., the cross-term is constant in $a$ and $a^\prime$, then the estimators share the same asymptotic variance.
\end{lemma}
...
In the following, we consider four simulation scenarios that extend the generative model described above. The first three scenarios concern estimation of the marginal proximal treatment effect $\beta_1^\star$. In each case, when the weighted and centered method is used, we set $f_t(S_t)=1$ in~\eqref{eq:generativemodel} (i.e., $S_{1t} = \emptyset$). We report average $\hat{\beta}_1$ point estimates, standard deviation and root mean squared error of $\hat{\beta}_1$, and 95\% confidence interval coverage probabilities across 1000 replicates. We vary the number of clusters and cluster size.  Confidence intervals are constructed based on standard errors that are corrected for the estimation of weights and/or small samples.  We compare the proposed cluster-based method (G-WCLS) to the approach by~\cite{Boruvkaetal} (WCLS). 
...
\begin{figure}
    ...
    \caption{{\bf Left:} Group size  ($G$) versus nominal coverage probability with fixed total sample size. {\bf Right:} Variance of random intercept $b_g$ versus nominal coverage probability.  
    % WCLS approach has severe undercoverage as the variance increases while the proposed G-WCLS approach maintains 95\% nominal coverage.
    }
    \label{fig:undercoverage}
\end{figure}

\noindent {\bf Simulation Scenario III}.  In the third scenario, we assume the treatment effect for an individual depends on the average state of all individuals in the cluster. Therefore, define the cluster-level moderator $\bar S_{t,g} = \frac{1}{G_g}\sum_{j=1}^{G_g} S_{t,j}$ and consider the generative model: 
\begin{equation*}
Y_{t+1,j} = (-0.2 + b_g +  \beta_{11}^*\bar S_{t,g}) \times (A_{t,j} -p_t(1|H_{t,j})) + 0.8 S_{t,j} + \epsilon_g +\epsilon_{t+1,j}
\end{equation*}
Table~\ref{tab:simresults} again presents the results. Again, the proposed estimator and achieves the nominal 95\% coverage probability while the WCLS method does not. 

\begin{table}[!th]
\centering
\begin{tabular}{c | ccccccc}
\hline
Scenario & Estimator & \# of Clusters & Cluster Size & Estimation & SE & RMSE & CP \\ \hline
\multirow{12}{*}{I} & G-WCLS & \multirow{2}{*}{25} & \multirow{2}{*}{10} & -0.198 & 0.035 & 0.036	 & 0.945 \\ 
& WCLS & & &  -0.200 & 0.036 & 0.035 & 0.956 \\  \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{25} & \multirow{2}{*}{25} & -0.199 & 0.022 & 0.023 & 0.948 \\ 
& WCLS & & &  -0.199 & 0.023 & 0.022 & 0.958 \\ \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{50} & \multirow{2}{*}{10} & -0.198 & 0.025 & 0.027 & 0.935 \\ 
& WCLS & & &  -0.198 & 0.026 & 0.026 & 0.944 \\ \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{50} & \multirow{2}{*}{25} & -0.198 & 0.016 & 0.016 & 0.950 \\ 
& WCLS & & &  -0.198 & 0.016 & 0.017 & 0.937 \\ \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{100} & \multirow{2}{*}{10} & -0.199 & 0.018 & 0.018 & 0.949 \\ 
& WCLS & & &  	-0.198& 0.018 & 0.019 & 0.949 \\ \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{100} & \multirow{2}{*}{25} &  -0.198 & 0.011 & 0.012  & 0.941 \\
& WCLS & & &  -0.199 & 0.011 & 	0.012 & 0.944 \\ \hline
\multirow{12}{*}{II} & G-WCLS & \multirow{2}{*}{25} & \multirow{2}{*}{10} & -0.199 & 0.070 & 0.076 & 0.935 \\ 
& WCLS & & &  -0.201 & 0.041 & 0.076 & 0.710 \\  \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{25} & \multirow{2}{*}{25} & -0.196 & 0.065 & 0.071 & 	0.933 \\ 
& WCLS & & &  -0.200 & 0.026 & 0.065 & 0.557 \\ \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{50} & \multirow{2}{*}{10} & -0.200 & 0.051 & 0.049 & 0.957 \\ 
& WCLS & & &  -0.200 & 0.029 & 0.052 & 0.723 \\ \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{50} & \multirow{2}{*}{25} & -0.200 & 0.047 & 0.049 & 0.947 \\ 
& WCLS & & &  -0.199 & 0.019 & 0.048 & 0.555 \\ \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{100} & \multirow{2}{*}{10} & -0.198  & 0.036 & 0.035 & 0.955 \\ 
& WCLS & & &  -0.198 & 0.021 & 0.037 & 0.718 \\ \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{100} & \multirow{2}{*}{25} & -0.198  & 0.033 & 0.035 & 0.942 \\ 
& WCLS & & &  -0.199 & 0.013 & 0.032 & 0.583 \\ \hline
\multirow{12}{*}{III} & G-WCLS & \multirow{2}{*}{25} & \multirow{2}{*}{10} & -0.199 & 0.070 & 0.073 & 0.948 \\ 
& WCLS & & &  -0.202 & 0.041 & 0.074 & 0.734 \\  \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{25} & \multirow{2}{*}{25} & -0.196 & 0.066 & 0.069 & 0.931 \\ 
& WCLS & & &  -0.200 & 0.026 & 0.068 & 0.563 \\ \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{50} & \multirow{2}{*}{10} & -0.198 & 0.051 & 0.052 & 0.941 \\ 
& WCLS & & &  -0.199 & 	0.029 & 0.052 & 0.742 \\ \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{50} & \multirow{2}{*}{25} & -0.199 & 0.047 & 0.048 & 0.946 \\ 
& WCLS & & &  -0.200 & 0.018 & 0.048 & 0.561 \\ \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{100} & \multirow{2}{*}{10} & -0.200 & 0.036 & 0.037 & 0.946 \\ 
& WCLS & & &  	-0.200 & 0.021 & 0.037 & 0.740 \\ \cdashline{2-8}
& G-WCLS & \multirow{2}{*}{100} & \multirow{2}{*}{25} & -0.201 & 0.034 & 0.033 & 0.956 \\ 
& WCLS & & &  -0.198 & 0.013 & 0.034 & 0.555 \\ \hline
\end{tabular}
\caption{Group weighted-centered least squares (G-WCLS) and weighted-least squares estimator (WCLS)  comparison over Scenarios I: ; II:; and III; \zw{change ``Estimation" to ``Bias" as the actual estimates are irrelevant to the point}} 
...
\begin{equation*}
    Y_{t+1,j} = (-0.2 + b_g +  \beta_{11} \bar S_{t,g}) \times \{A_{t,j} -p_t(1|H_{t,j})\}+ 0.8 S_{t,j} +TE_{j,t} +\epsilon_g +\epsilon_{t+1,j} 
\end{equation*}
This model implies a marginal indirect effect equal to $\beta_1^{(IE)} = \beta_{20} = -0.1$. Table~\ref{tab:simresults_indirect} presents the results.  We see that the proposed indirect estimator exhibited no bias and achieved the nominal coverage probability.

\begin{table}[!th]
\centering
\begin{tabular}{c | cccccc}
\hline
Scenario & \# of Clusters & Cluster Size & Estimation & SD & RMSE & CP \\ \hline
...
\end{tabular}
\caption{Group weighted-centered least squares (G-WCLS) for estimation of indirect effects \zw{Similar as before; suggest changing "Estimates" to "Bias"}} 
...
& \multirow{2}{*}{G-WCLS} & $\beta_0$ & 0.350 & 0.103 & 3.401 & 0.001 \\ 
& & $\beta_1$ & -0.053 & 0.014 & -3.868 & 0.000 \\ \cline{2-7}
& \multirow{3}{*}{G-WCLS} & $\beta_0$ & -0.238 & 0.282 & -0.842 & 0.401 \\ 
...
& \multirow{2}{*}{G-WCLS} & $\beta_0$ & 0.622 & 0.384 & 1.618 & 0.108 \\ 
& & $\beta_1$ & -0.031 & 0.019 & -1.580 & 0.117 \\ \cline{2-7}
& \multirow{3}{*}{G-WCLS} & $\beta_0$ & -2.095 & 1.248 & -1.678 & 0.094 \\ 
& & $\beta_1$ & -0.034 & 0.019 & -1.745 & 0.083 \\ 
& & $\beta_2$ & 0.143 & 0.062 &2.330 & 0.020 \\ \hline
\end{tabular}
\caption{Moderation analysis for the effect of notifications on average weekly mood scores and log step counts respectively in IHS. WCLS estimates use the estimator in~\cite{Boruvkaetal}, while the G-WCLS estimates use our proposed estimator.} 
...
Here we consider the causal excursion effect in the presence of a priori known clusters.  We redefine the causal excursion effect to naturally account for cluster information and described both direct and indirect excursion effects to account for potential interference.  The effects described in this paper are most important when using MRT data to build optimized JITAIs for deployment in an mHealth package. Specifically, the estimation procedure for the direct excursion effect accounts for cluster-level correlation in the proximal outcomes which helps the scientific team avoid making erroneous conclusions about intervention effectiveness using standard MRT methods.  Moreover, estimation of indirect effects allows the scientific team to answer questions about impact of interventions on other members of the same cluster.  Use of these methods provides empirical evidence for the scientific team to include or exclude intervention components that may have had unanticipated second order effects, or potentially lead to novel ways to improve the intervention component by revising the intervention to more explicitly account for cluster-level interference.  While this work represents a major step forward in the analysis of micro-randomized trial data, further work is required.  Specifically, extensions to be considered future work include accounting for overlapping communities and/or network (rather than cluster-only) structure~\cite{Ogburn2014,Mealli2019}, penalization of the working model to allow for high-dimensional moderators, and a method to use the proposed approach to form warm-start policies at the individual level while accounting for group level information~\cite{Luckett2020}.  
...
% & $\beta_2$ & 0.143 & 0.062 &2.330 & 0.020 \\ \hline
% \end{tabular}
% \caption{Moderation analysis for the effect of notifications on average weekly log step counts in IHS. WCLS estimates use the estimator in~\cite{Boruvkaetal}, while the G-WCLS estimates use our proposed estimator.} 
...
\multirow{2}{*}{G-WCLS} & $\beta_0$ & 1.193 & 0.454 & 2.627 & 0.011 \\ 
& $\beta_1$ & -0.061 & 0.023 & -2.696 & 0.009 \\ \hline
\multirow{3}{*}{G-WCLS} & $\beta_0$ & -1.912 & 1.379 & -1.386 & 0.171  \\ 
& $\beta_1$ & -0.067 & 0.023 & -2.948 & 0.004 \\ 
& $\beta_2$ & 0.162 & 0.065 & 2.487 & 0.015 \\ \hline
\end{tabular}
\caption{Moderation analysis for the effect of notifications on average weekly log sleep minutes in IHS. WCLS estimates use the estimator in~\cite{Boruvkaetal}, while the G-WCLS estimates use our proposed estimator.} 
...
\begin{align}
    \sum_{t=1}^T \left( Y_{t+1,J} - g_t(H_t)^\prime \alpha -  (1-A_{t,J}) (A_{t,J^\prime} - &\tilde p_t^\star (1 \given S_t;\hat\rho) ) f_t (S_t)^\prime \beta \right) I_{t,J}I_{t,J^\prime}W_{t,J, J^\prime}(A_{t,J},A_{t,J^\prime},H_t; \hat\eta,\hat\rho) \nonumber\\
    &\begin{pmatrix}
  g_t(H_t) \\ 
...
%  \hat \alpha \\ 
%  \hat \beta
%\end{pmatrix} = \left\{ \P_n \dot U_W(\hat\eta,\hat\rho) \right\}^{-1}\P_n \sum_{t=1}^T I_{t,J}I_{t,J^\prime} W_{t,J, J^\prime}(A_{t,J},A_{t,J^\prime},H_t; \hat\eta &,\hat\rho) Y_{t+1,J} \nonumber \\
%&\begin{pmatrix}
%  g_t(H_t) \\ 
...
%\begin{align*}
%    0 &= \E \sum_{t=1}^T \left( Y_{t+1,J} - g_t(H_t)^T\alpha^\prime -  (1-A_{t,J}) (A_{t,J^\prime} - \tilde p_t^\star (1 \given S_t;\rho^\star) ) f_t (S_t)^T \beta^\prime \right) I_{t,J}I_{t,J^\prime} \nonumber \\ 
...
%    &=  \E \sum_{t=1}^T \left(\E \left[Y_{t+1} \given{A_{t,J},A_{t,J^\prime}, H_t ,I_{t,J}I_{t,J^\prime}=1} \right]- g_t(H_t)^T\alpha^\prime-(1-A_{t,J}) (A_{t,J^\prime} - \tilde p_t^\star (1 \given S_t;\rho^\star) ) f_t (S_t)^T \beta^\prime \right) \nonumber \\
...
%    &= \E \sum_{t=1}^T \sum_{a^\prime \in \{0,1\}} \Big(\E \left[Y_{t+1} \given{A_{t,J}=0,A_{t,J^\prime}=a^\prime, H_t ,I_{t,J}I_{t,J^\prime}=1} \right]- g_t(H_t)^T\alpha^\prime- \nonumber \\
%    &  ~~~~~~~~~~~~~~~~~~~~  (a^\prime - \tilde p_t^\star (1 \given S_t;\rho^\star) ) f_t (S_t)^T \beta^\prime \Big) I_{t,J}I_{t,J^\prime}\tilde p_t (0,a^\prime \given S_t;\rho^\star)(a^\prime - \tilde {p}^\star_t (1 \given S_t;\rho^\star) ) f_t (S_t)
%\end{align*}
%
%where the last equality averages out over $A_{t,J^\prime}$. The above simplifies to:
%
%\begin{align*}
%    0 &= \E \sum_{t=1}^T \sum_{a^\prime \in \{0,1\}} \Big(\E \left[Y_{t+1} \given{A_{t,J}=0,A_{t,J^\prime}=a^\prime, H_t ,I_{t,J}I_{t,J^\prime}=1} \right]- g_t(H_t)^T\alpha^\prime- \nonumber \\
%    &  ~~~~~~~~~~~~~~~~~~~~  (a^\prime - \tilde p_t^\star (1 \given S_t;\rho^\star) ) f_t (S_t)^T \beta^\prime \Big) I_{t,J}I_{t,J^\prime}\tilde p_t (0,a^\prime \given S_t;\rho^\star)(a^\prime - \tilde {p}^\star_t (1 \given S_t;\rho^\star) ) f_t (S_t) \nonumber \\
%    &= \E \sum_{t=1}^T  \Big(\E \left[Y_{t+1} \given{A_{t,J}=0,A_{t,J^\prime}=1, H_t ,I_{t,J}I_{t,J^\prime}=1} \right]- g_t(H_t)^T\alpha^\prime- \nonumber \\
%    &  ~~~~~~~~~~~~~~~~~~~~  (1 - \tilde p_t^\star (1 \given S_t;\rho^\star) ) f_t (S_t)^T \beta^\prime \Big) I_{t,J}I_{t,J^\prime}\tilde p_t (0,1 \given S_t;\rho^\star)(1 - \tilde {p}^\star_t (1 \given S_t;\rho^\star) ) f_t (S_t) \nonumber \\
%    & ~~~~ +\E \sum_{t=1}^T \Big(\E \left[Y_{t+1} \given{A_{t,J}=0,A_{t,J^\prime}=0, H_t ,I_{t,J}I_{t,J^\prime}=1} \right]- g_t(H_t)^T\alpha^\prime- \nonumber \\
%    &  ~~~~~~~~~~~~~~~~~~~~  ( - \tilde p_t^\star (1 \given S_t;\rho^\star) ) f_t (S_t)^T \beta^\prime \Big) I_{t,J}I_{t,J^\prime}\tilde p_t (0,0 \given S_t;\rho^\star)( - \tilde {p}^\star_t (1 \given S_t;\rho^\star) ) f_t (S_t) \nonumber \\
...
%    &  ~~~~~~~~~~~~~~~~~~~~  -f_t (S_t)^T \beta^\prime \Big)  f_t (S_t) \gamma(\eta^\star,\rho^\star) I_{t,J}I_{t,J^\prime} 
%\end{align*}
%
...
%   &\E \left[Y_{t+1} \given{A_{t,J}=0,A_{t,J^\prime}=0, H_t ,I_{t,J}I_{t,J^\prime}=1} \right] \mid S_t,I_{t,J}I_{t,J^\prime}=1\Big]  -f_t (S_t)^T \beta^\prime \Big) 
%\end{align*}
%
%Thus 
%\begin{align}
...
%where
%\[
%\left[\E \dot U_W(\eta^\star,\rho^\star) \right]_{(2,2)} =\E \sum_{t=1}^T  f_t (S_t)f_t (S_t)^T \gamma(\eta^\star,\rho^\star) I_{t,J}I_{t,J^\prime}
%\]
%\end{proof}
%Recall Assumption 4.3, we can see from equation \eqref{eq:estimand_beta}, when the model is incorrectly specified, then the choice of $\tilde p_t(1\mid S_t;\rho^\star)$ determines the estimand. In our simulation setting, we chose $\tilde p_t(1\mid S_t;\rho^\star)$ to be a constant, then the limit in probability $\hat\beta$ is given by
%\begin{align}
%    \beta^\prime =\left[\E  \sum_{t=1}^T  f_t (S_t)f_t (S_t)^T I_{t,J}I_{t,J^\prime} \right]^{-1} &\E \Bigg[\sum_{t=1}^T  f_t (S_t)  I_{t,J}I_{t,J^\prime}  \E \Big[\E \left[Y_{t+1} \given{A_{t,J}=0,A_{t,J^\prime}=1, H_t ,I_{t,J}I_{t,J^\prime}=1} \right]  \nonumber \\ 
%   &-\E \left[Y_{t+1} \given{A_{t,J}=0,A_{t,J^\prime}=0, H_t ,I_{t,J}I_{t,J^\prime}=1} \right] \mid S_t,I_{t,J}I_{t,J^\prime}=1\Big]  \Bigg]
%\end{align}
%
%In the case in which $f_t(S_t)=1$ (i.e., $S_t = \emptyset$) then the scalar estimand is simply and average (weighted by availability) of proximal treatment effects:
%\begin{align*}
%  \Big(\sum_{t=1}^T \E \left[ I_{t,J}I_{t,J^\prime}\right] \Big)^{-1} &\sum_{t=1}^T \E \left[ I_{t,J}I_{t,J^\prime}\right] \E \Big[\E \left[Y_{t+1} \given{A_{t,J}=0,A_{t,J^\prime}=1, H_t ,I_{t,J}I_{t,J^\prime}=1} \right]  \nonumber \\ 
%   &-\E \left[Y_{t+1} \given{A_{t,J}=0,A_{t,J^\prime}=0, H_t ,I_{t,J}I_{t,J^\prime}=1} \right] \mid I_{t,J}I_{t,J^\prime}=1\Big]  
%\end{align*}
